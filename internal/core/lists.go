...
func (c *Core) QueryLists(searchStr, typ, optin string, names, tags []string, orderBy, order string, getAll bool, permittedIDs []int, offset, limit int) ([]models.List, int, error) {
	...
	if err := c.db.Select(&out, stmt, 0, "", queryStr, typ, optin, pq.StringArray(tags), getAll, pq.Array(permittedIDs), offset, limit, pq.StringArray(names)); err != nil {
		c.log.Printf("error fetching lists: %v", err)
...
func (c *Core) GetList(id int, uuid string) (models.List, error) {
	...
	if err := c.db.Select(&res, stmt, id, uu, queryStr, "", "", pq.StringArray{}, true, nil, 0, 1, nil); err != nil {
		c.log.Printf("error fetching lists: %v", err)
